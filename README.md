...
`supabase/migrations/202609070004_member_like_milestones.sql` を適用してデプロイします。追加のSlack設定は不要です。
各投稿者の受け取ったいいねの合計が **10 → 20 → 50 → 100 → 200 → 500 → 1,000 → 2,000 → 5,000…**
（1・2・5 × 10の累乗）に到達すると、本人へDMを送ります。日報単位の通知とは別です。

累計はランキングの「受け取ったいいね」と同じく、現在残っている全日報のいいねの合計です。
同じ人が別の日報に押したいいねはそれぞれ1件、同じ日報のSlackスタンプとWebいいねは合計1件です。
解除・削除で合計が減っても到達記録は保持し、再到達による再通知はしません。
導入前のいいねも対象で、未通知の複数の節目は最高の到達数を1通にまとめます。
既存の送信処理・再試行を使い、管理画面のDM通知履歴に「個人の累計いいね」として宛先・本文・状態を表示します。

文面例:

> 🎉 あなたの日報が受け取った累計いいねが1,000件に到達しました！
> いつも活動の共有をありがとうございます！
> 自分の活動を見る
...
`supabase/migrations/202609080001_video_attachments.sql` を適用してからアプリをデプロイします。カスタムバケットを使用する場合は、SQLのバケットIDを `SUPABASE_STORAGE_BUCKET` に合わせてください。Supabase全体のファイル上限も50MiB以上が必要です。

JPEG・PNG・WebPは1件5MiB、MP4・MOV・WebMは1件50MiB、添付合計は100MiBまでです。動画は非公開ストレージへ直接アップロードし、日報詳細で再生できます。コーデックによってブラウザーで再生できない動画は「動画を開く・ダウンロード」から確認できます。Slack・Notionでは日報ページから動画を確認する案内を表示します。

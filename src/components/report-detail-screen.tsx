        ...
        <div className="report-article__body">
          ...
          {report.attachments.length ? (
            <section aria-labelledby="images-heading" className="report-section report-section--media">
              <div className="report-section__heading"><span>MEDIA</span><h2 id="images-heading">画像・動画</h2></div>
              <div className="report-images">
                {report.attachments.map((attachment) => attachment.signedUrl ? (
                  <figure key={attachment.id || attachment.storagePath}>{isVideo(attachment.mimeType) ? <><video aria-label={attachment.altText || attachment.filename} controls playsInline preload="metadata" src={attachment.signedUrl} /><a href={attachment.signedUrl} rel="noreferrer" target="_blank">動画を開く・ダウンロード</a></> : <img alt={attachment.altText || "日報に添付された画像"} src={attachment.signedUrl} />}<figcaption>{attachment.altText || attachment.filename}</figcaption></figure>
                ) : (
                  <div className="report-image-placeholder" key={attachment.id || attachment.storagePath}><span>添付ファイル</span><p>{attachment.filename}</p></div>
                ))}
              </div>
            </section>
          ) : null}
